...
function reduceNum(skuId, quantity){
	...
	//alert(skuId+"---"+quantity+"----"+jsonObj.stock);
	if(jsonObj.result == "no"){
		var newQuantity = quantity - 1;
		alert("库存不足"+ newQuantity + "个,仅有"+ jsonObj.stock + "个");
	...
	if(quantity == 1){
		if(confirm("是否从购物车中删除该商品?")){
			window.location.href = "${path}/cart/deleteItem.do?skuId="+skuId;
		}
	}else{
		window.location.href = "${path}/cart/reduceNum.do?skuId="+skuId;
	}
		...
		<div class="ofc pb40">
			...
			<tbody>
				<c:forEach items="${cartList }" var="cart">
					...
					<td>
						<a href="javascript:void(0);" title="减" class="inb arr" onclick="reduceNum(${cart.skuId}, ${cart.quantity })" >-</a><input type="text" class="txts" size="1" name="" value="${cart.quantity }" /><a href="javascript:void(0);" title="加" class="inb arr">+</a></td>
...
<div id="loginAlert" class="alt login" style="display:none">
	...
	<div class="cont">
		...
		<li><label>密码：</label>
			<span class="bg_text"><input type="password" vld="{required:true}" maxlength="20" name="ord" id="password" value="" reg1="^.{6,20}$" desc="密码长度范围为6-20，允许为中英文、数字或特殊字符！" /></span>
		</li>
		<li>
